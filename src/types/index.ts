...
export interface ProductCard {   // интерфейс карточки товара, содержащий в себе:
   ...
   price: string | number  // цена карточки, т.е товара
}

export interface CloseModalButton { // решил вынести в отдельный интерфейс кнопку закрытия, т.к присутствует в каждой модалке
   closeButton: string | HTMLButtonElement // кнопка закрытия модалки
}

export interface ProductAddPopup extends ProductCard, CloseModalButton {  // этот интерфейс модального окна, содержащий все тоже, что и карточка товара + :
...
export interface OrderPopup extends CloseModalButton { // модальное окно выбранных заказов - корзина
   nameModal: string // т.е название модального окна - корзина
   selectedProduct: {  // выбранный продукт, представим его массивом объектов, в котором:
...
export interface OrderPaymentPopup extends CloseModalButton { // модальное окно оплаты заказа
   nameModal: string // название модального окна - оплата заказа
   orderPaymentSelectionButton: {  // кнопки выбора оплаты:
      buttonOnline: 'Онлайн' 
...
export interface ContactInformationUserPopup extends CloseModalButton { // модальное окно контактных данных пользователя
   inputEmail: HTMLInputElement
   inputTel: HTMLInputElement
   payOrderButton: HTMLButtonElement
}

export interface OrderConfirmedPopup extends CloseModalButton { // ну и модальное окно статуса заказа
   image: string
   orderStatus: string
   priceOrder: number
...
export interface ProductModel<T> { // интерфейс действий с товаром, такие как:
